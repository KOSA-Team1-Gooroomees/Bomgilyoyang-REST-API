...
export default function Register() {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            setError("회원가입 실패: " + err.response?.data?.message || '정보가 올바르지 않습니다.');
        } finally {
            setIsLoading(false);
        }
    }
    ...
    const handleConfirmPwdChange = (confirmPwd) => {
    ...
    function validatePassword() {
        ...
        if (!val) {
            passwordMsg.classNameList.add('hidden');
            return false;
        }

        passwordMsg.classNameList.remove('hidden');
        if (passwordPattern.test(val)) {
            passwordMsg.textContent = '안전한 비밀번호입니다.';
            passwordMsg.classNameName = 'text-xs mt-1 mb-3 px-1 text-emerald-600 font-medium';
            return true;
        } else {
            passwordMsg.textContent = '비밀번호는 영문, 숫자를 조합하여 8~16자로 입력해주세요.';
            passwordMsg.classNameName = 'text-xs mt-1 mb-3 px-1 text-rose-500 font-medium';
            return false;
        }
    }
    ...
    //     if (!confirmVal) {
    //         confirmPasswordMsg.classNameList.add('hidden');
    //         return false;
    //     }

    //     confirmPasswordMsg.classNameList.remove('hidden');
    //     if (pwVal === confirmVal) {
    //         confirmPasswordMsg.textContent = '비밀번호가 일치합니다.';
    //         confirmPasswordMsg.classNameName = 'text-xs mt-1 mb-4 px-1 text-emerald-600 font-medium';
    //         return true;
    //     } else {
    //         confirmPasswordMsg.textContent = '비밀번호가 일치하지 않습니다.';
    //         confirmPasswordMsg.classNameName = 'text-xs mt-1 mb-4 px-1 text-rose-500 font-medium';
    //         return false;
    //     }
    // }
    ...
    function checkDuplicatedEmail() {
        ...
        if (!emailVal) {
            emailMsg.classNameList.remove('hidden');
            emailMsg.textContent = '이메일을 입력해주세요.';
            emailMsg.classNameName = 'text-xs mt-1 mb-3 px-1 text-rose-500 font-medium';
            emailInput.focus();
            return;
        }
        ...
        fetch(`/api/auth/check-email?email=${encodeURIComponent(emailVal)}`)
            ...
            .then(isDuplicated => {
                ...
                if (isDuplicated) {
                    ...
                    emailMsg.classNameName = 'text-xs mt-1 mb-3 px-1 text-emerald-600 font-medium';
                    isEmailChecked = true;
        ...
        <>
            <div className="flex-1 flex items-center justify-center relative overflow-hidden px-4 py-10">
                ...
                <div className="relative z-10 w-full max-w-[400px] px-5 mt-[-50px] ">
                    ...
                    <div className="rounded-[20px] shadow-lg p-6 relative z-10 mt-10 bg-[url('/images/login-background.png')] bg-cover bg-bottom ">
                        ...
                        {/* <div th:if="${error}" className="mb-4 p-3.5 bg-red-50 border border-red-200 text-red-600 rounded-xl text-xs text-center font-medium">
                                    <span th:text="${error}">오류 메시지</span>
                                </div> */}
                        {/* <div id="clientErrorBanner" className="hidden mb-4 p-3.5 bg-red-50 border border-red-200 text-red-600 rounded-xl text-xs text-center font-medium">
                                    <span id="clientErrorMessage">오류 메시지</span>
                                </div> */}

                        <form id="signupForm" onClick={handleRegister}>
                            ...
                            <div className="relative mb-3">
                                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                    <svg className="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 8l7.89 5.26a2 2 0 002-2v10a2 2 0 002 2z" />
                                    </svg>
                                </div>
                                ...
                                <button type="button" id="checkEmailBtn"
                                    className="absolute inset-y-1.5 right-1.5 px-3 bg-brand-green hover:bg-brand-greenHover text-white text-xs font-semibold rounded-lg transition-colors duration-200"
                                    onClick={checkDuplicatedEmail}>
                                    중복 확인
                                </button>
                            </div>
                            <div id="email-validation-msg" className="hidden text-xs mt-1 mb-3 px-1 font-medium"></div>
                            ...
                            <div className="relative mb-3">
                                ...
                                <input type="password" name="password" id="password" placeholder="비밀번호를 입력하세요"
                                    ...
                                    required />
                                <button type="button" id="togglePassword" className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-gray-400 hover:text-gray-600 focus:outline-none">
                                    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                </button>
                            </div>
                            <div id="password-validation-msg" className="hidden text-xs mt-1 mb-3 px-1 font-medium"></div>
                            ...
                            <div className="relative mb-3">
                                ...
                                <input type="password" name="confirmPassword" id="confirmPassword" placeholder="비밀번호 확인"
                                    ...
                                    required />
                                <button type="button" id="toggleConfirmPassword" className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-gray-400 hover:text-gray-600 focus:outline-none">
                                    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                </button>
                            </div>
                            {/* <div id="confirm-password-validation-msg" className="hidden text-xs mt-1 mb-4 px-1 font-medium"></div> */}
                            {confirmPwd && (
                                register.password === confirmPwd ? (
                                    <p 
                                        style={{ color: 'green', fontSize: '14px', margin: '5px 0 0 0' }}>
                                        사용 가능한 비밀번호입니다.
                                    </p>
                                ) : (
                                    <p 
                            ...
                            <button type="submit" className="w-full bg-brand-green hover:bg-brand-greenHover text-white font-medium py-3.5 rounded-xl transition-colors duration-200 shadow-sm">
                                회원가입
                            </button>
                        </form>
                        ...
                        <a th:href="@{/oauth2/authorization/kakao}" className="w-full bg-brand-kakao hover:bg-brand-kakaoHover text-[#3C1E1E] font-medium py-3.5 rounded-xl flex items-center justify-center gap-2 transition-colors duration-200 text-sm shadow-sm">
                        ...
                        <div className="mt-7 text-center text-[13px]">
                            ...
                            <a th:href="@{/login}" className="text-[#4A8A54] font-medium hover:underline ml-1">로그인</a>
                        </div>
                    </div>
                </div>
            </div>
...
//         if (!val) {
//             emailMsg.classNameList.add('hidden');
//             return false;
//         }
...
//             emailMsg.classNameName = 'text-xs mt-1 mb-3 px-1 text-blue-600 font-medium';
//             return true;
//         } else {
//             emailMsg.textContent = '올바른 이메일 형식을 입력해주세요.';
//             emailMsg.classNameName = 'text-xs mt-1 mb-3 px-1 text-rose-500 font-medium';
